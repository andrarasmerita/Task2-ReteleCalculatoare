...
while True:
    ...
    if comanda in ['PUBLISH', 'DELETE', 'LIST'] and not este_conectat:
    ...
    if comanda == 'CONNECT':
        ...
        if not argumente:
            print("EROARE: Trebuie furnizat un mesaj. Exemplu: PUBLISH salut")
        else:
            raspuns = trimite_comanda(intrare)
            print(raspuns)
        ...
        if not argumente.isdigit():
            print("EROARE: ID invalid. Trebuie sa fie un numar intreg. Exemplu: DELETE 1")
        else:
            raspuns = trimite_comanda(intrare)
            print(raspuns)
